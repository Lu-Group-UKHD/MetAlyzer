...
metalyzer_se <- MetAlyzer::read_metidq(conc_file_path = fpath,
                                       silent = TRUE)
...
R packages dedicated for quality control and differential analysis, we decided to not focus on this. Never the less, we will showcase how you can easily perform differential analysis using 
the ***MetaProViz*** R package [@Schmidt2025]. If you are not familiar with using R, you can use our shiny app, which will do the whole process from start to finish (@ADD LINK).\
...
```{r diffres, eval=FALSE,echo=TRUE, results='hide'}
## -------------------------------------------------------------------------- ##
...
# using external R-packages like MetaProViz (https://saezlab.github.io/MetaProViz/articles/standard-metabolomics.html#run-metaproviz-analysis) 
