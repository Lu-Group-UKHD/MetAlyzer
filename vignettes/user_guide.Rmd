...
For further demonstration we'll use a demo dataset from Biocrates, featuring metabolite measurements across two groups (MxP®Quant 500 kit). This dataset is included in the package.
:::
...
metalyzer_se <- MetAlyzer::read_metidq(file_path = fpath, silent = TRUE)
...
::: {style="text-align: justify"}
For the calculation of the Log2FC DataFrame we used the MetaProViz package[MetaProViz].
We'll use the example differential results included in the package (toy_diffres()). 
...
## -------------------------------------------------------------------------- ##
##  Skip this part if you already know how to generate a Log2 Fold Change df  ##
## -------------------------------------------------------------------------- ##

## Sample Info
col <- SummarizedExperiment::colData(metalyzer_se)
col_metadata_df <- data.frame(col@listData)
col_metadata_df$RowNames <- col@rownames

sample_info <- col_metadata_df %>%
  tibble::column_to_rownames("RowNames") %>%
  dplyr::rename("Conditions" = "Sample.Description")

## Input Concentration values
intra_m <- as.data.frame(t(SummarizedExperiment::assay(metalyzer_se))) # rownames must match the rownames of the SampleInfo

# Preprocessing
preprocessing_results <- MetaProViz::PreProcessing(InputData = intra_m,
                                                  SettingsFile_Sample = sample_info,
...
intra_preprocessed <- preprocessing_results[["DF"]][["Preprocessing_output"]]

## Mapping Info
row_metadata_df <- data.frame(SummarizedExperiment::rowData(metalyzer_se))

mapping_info <- row_metadata_df

# Differential Metabolite Analysis
dma <- MetaProViz::DMA(InputData = intra_preprocessed[, -c(1:14)], #filter non-numeric columns
                       SettingsFile_Sample = sample_info,
                       SettingsInfo = c(Conditions="Conditions", Numerator="Group 1" , Denominator = "Group 2"),
                       SettingsFile_Metab = mapping_info,
                       StatPval = "lmFit",
                       StatPadj = "fdr")

dma_group1_vs_group2 <- dma[["DMA"]][["Group 1_vs_Group 2"]]

diffres_df <- dma_group1_vs_group2 %>%
  select(
...
MetaProViz::VizVolcano(InputData = T14_node_expanded, yCutoff = 0.05, PlotName = "Vulcano Plot Group 1 vs Group 2")
```
