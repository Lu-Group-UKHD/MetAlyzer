...
Search the text for "@ADD" as at those points additions are still needed!
...
::: {style="text-align: justify"}
***MetAlyzer's*** primary entry point is the `MetAlyzer::read_metidq()` function, which reads MetIDQ™ .xlsx output files (Fig. 1). In detail, you can export those files by using @ADD button/MetIDQ path info!\ 
...
For further demonstration we'll use a demo dataset kindly provided by BIOCRATES Life Sciences AG, featuring metabolite measurements across two groups (MxP®Quant 500 kit). @ADD info about the conditions and the biology - what would we expect from the biological system?. This dataset is included in the ***MetAlyzer*** R package as the example data.
:::
...
::: {style="text-align: justify"}
The ***MetAlyzer*** R package does not include differential analysis itself, as it's aim is to import Biocrates files and visualize the results. Given the many tools and 
R packages dedicated for quality control and differential analysis, we decided to focus on the visualization part. Never the less, we will showcase how you can easily perform 
differential analysis using the ***MetaProViz*** R package [@MetaProViz]. If you are not familiar with using R, you can use our shiny app, which will do the whole process from start to finish (@ADD LINK).\
...
<div class="vscroll-plot">
```{r diffres, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
## -------------------------------------------------------------------------- ##
##  Skip this part if you already have generated differential analysis results ##
## -------------------------------------------------------------------------- ##

# Ensure that you have performed all needed processing, normalisation and QC steps in MetIDQ™ or 
# using external R-packages like MetaProViz before running the differential analysis.

##----- Create InputDF
assay_matrix <- metalyzer_se %>%
  assay() %>%         # Extract the "counts" assay
  `colnames<-`(colnames(metalyzer_se))%>%
  t()%>%
  as.data.frame()

sample_info <- metalyzer_se %>%
  SummarizedExperiment::colData() %>%
  as.data.frame() %>%
  dplyr::rename("Conditions" = "Sample.Description")

feature_info <- metalyzer_se %>%
  rowData() %>%
  as.data.frame()
  
##----- Processing
#@ADD: I think we should rather export the processed file from MetIDQ and note down which steps had been done in the software. I would also extend the sample metainformation (e.g. replicate, tissue type etc.) and the feature information (metabolite IDs). At the moment the samples just have numbers, but I think we should give them ore meaningfull names including their Condition, replicate number etc. Ask @Gernot/Biocrates
# For function details visit MetaProViz: https://saezlab.github.io/MetaProViz/reference/processing.html
processing_res <- MetaProViz::processing(data= assay_matrix, 
                                         metadata_sample= sample_info, 
                                         metadata_info = c(Conditions = "Conditions"))

processedDF <- processing_res[["DF"]][["Preprocessing_output"]]%>%
  dplyr::select(-c(1:14))

##------ Differential Analysis 
# For function details visit MetaProViz: https://saezlab.github.io/MetaProViz/reference/dma.html
dma_res <- MetaProViz::dma(data=processedDF , #we need to remove columns that do not include metabolite measurements
                           metadata_sample=sample_info, #only maintain the information about condition and replicates
                           metadata_info = c(Conditions="Conditions", 
                                             Numerator="Group 1", 
                                             Denominator = "Group 2"),
                           metadata_feature = data.frame(SummarizedExperiment::rowData(metalyzer_se)), # Adds metadata for the metabolites such as metabolite classes
                           pval ="lmFit",
                           padj="fdr")

dma_group1_vs_group2 <- dma_res[["dma"]][["Group 1_vs_Group 2"]]%>%
  dplyr::select(
...
T14_node_expanded <- tidyr::separate_rows(T14_node,
                                          Metabolites, values, stat,
                                          sep = "\\s*;\\s*") %>%
                     tibble::column_to_rownames("Metabolites") %>%
                     dplyr::rename(p.adj = stat,
                                   Log2FC = values) %>%
...
:::
For easy visualisation of the summarized nodes, you can use the MetaProViz Volcano Plot (package) [@MetaProViz].
:::

```{r vulcano, echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.height=4.5, fig.align="left"}
# https://saezlab.github.io/MetaProViz/reference/viz_volcano.html
MetaProViz::viz_volcano(data = T14_node_expanded, 
                        select_label = NULL,
                        plot_name = "T14 Node of Group 1 vs Group 2")
```
