...
```{r}
# Load biocrates-exported data
metabObj <- MetAlyzer::read_metidq('./inst/extdata/Metalyzer_demodataset_biocratesMxPQuant500XL_2025-04.xlsx',
                                   sheet = 1, silent = T)
...
Make plots for data distribution and missing patterns of UNPROCESSED data, which will be included in Figure 2A&B
```{r message=F}
...
processDist
# Save plot
# ggsave(filename = 'path_to_save', dpi = 600, units = 'cm', width = 38, height = 12)
```

Do differential analysis and visualize results via volcano, scatter, and network plots\
Customize themes of MetAlyzer::plot_scatter and MetAlyzer::plot_network (e.g., font size)\
Note that these two functions were modified from older but functional version (just run chunk below).
```{r, include=F}
...
demoRes_ABCD
# ggsave(filename = 'path_to_save', plot = demoRes_ABCD, dpi = 600, units = 'cm', width = 38, height = 52)
```
...
demoRes_E
# ggsave(filename = 'path_to_save', plot = demoRes_E, dpi = 600, units = 'cm', width = 38, height = 32)
```
...
# ggsave(filename = 'path_to_save', plot = suppl_hilightVulcano, dpi = 600, units = 'cm', width = 38, height = 40)
```
